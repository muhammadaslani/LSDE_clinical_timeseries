...
# No-context dispatch: drift_aug receives (x, u_t) instead of (x, u_t, c_t)
function (de::SDE)(x::AbstractArray, u::Union{Nothing,AbstractArray}, c::Nothing, ts::AbstractArray, p::ComponentVector, st::NamedTuple)
    u_cont1(t) = interp!(ts, u, t, Val(:linear))
    u_cont2(t) = interp!(ts, u, t, Val(:linear))

    function μ_augmented(x, p, t)
        u_t = u_cont1(t)
        return de.drift_aug((x, u_t), p, st.drift_aug)[1]
    end
...
# No-context dispatch for sample_augmented
function sample_augmented(de::SDE, init_map, solver, px₀, u, c::Nothing, ts, p, st, n_samples, dev; kwargs...)
    tspan = (ts[1], ts[end])
    u_cont(t) = interp!(ts, u, t, Val(:linear))

    x₀ = init_map(sample_rp(px₀), p.init_map, st.init_map)[1]

    μ(x, p, t) = de.drift_aug(vcat(x, u_cont(t)), p.drift_aug, st.dynamics.drift_aug)[1]
    σ(x, p, t) = de.diffusion(x, p.diffusion, st.dynamics.diffusion)[1]
