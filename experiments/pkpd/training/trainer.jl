...
function slice_data(data::NTuple{14}, indices)
...
function kfold_train(data, dims, n_folds, rng, config_path, model_type, timepoints, loss_fn, eval_fn, forecast_fn, viz_fn)
    ...
    val_config = merge(model_cfg["validation"], train_cfg["validation"])
    ...
    for fold_idx in 1:n_folds
        ...
        data_obs    = test_data[1:7]
        future_true = test_data[8:14]
        u_for_test  = test_data[8]
        Ex, Ey_pred = forecast_fn(model, θ_trained, st, data_obs, u_for_test, timepoints, val_config)
        perf        = eval_forecast(future_true, (Ex, Ey_pred))
        ...
        (y₁_acc, y₁_npe), (ŷ₂_rmse, ŷ₂_crps), (ŷ₂_count_rmse, ŷ₂_count_nll) = perf
        @info @sprintf("Fold %d completed: Acc=%.4f, NPE=%.4f, T-RMSE=%.4e, T-CRPS=%.4e, C-RMSE=%.4e, C-NLL=%.4e",
            fold_idx, y₁_acc, y₁_npe, ŷ₂_rmse, ŷ₂_crps, ŷ₂_count_rmse, ŷ₂_count_nll)
    end

    avg_acc    = mean(p[1][1] for p in performances)
    avg_npe    = mean(p[1][2] for p in performances)
    avg_t_rmse = mean(p[2][1] for p in performances)
    avg_t_crps = mean(p[2][2] for p in performances)
    avg_c_rmse = mean(p[3][1] for p in performances)
    avg_c_nll  = mean(p[3][2] for p in performances)
    elapsed    = time() - start_time

    @info @sprintf("K-Fold Results: Acc=%.4f, NPE=%.4f, T-RMSE=%.4e, T-CRPS=%.4e, C-RMSE=%.4e, C-NLL=%.4e",
        avg_acc, avg_npe, avg_t_rmse, avg_t_crps, avg_c_rmse, avg_c_nll)
    @info "Total training time: $(round(elapsed, digits=2))s ($(round(elapsed/60, digits=2)) min)"

    return models, trained_params, states, performances, config
end
