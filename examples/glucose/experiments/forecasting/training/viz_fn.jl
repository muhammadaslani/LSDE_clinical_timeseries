...
function viz_fn(obs_timepoints, for_timepoints, obs_data, future_true_data, forecasted_data, normalization_stats)
    ...
    # Unpack data
    _, _, x_o, y_o, mask_o = obs_data
    _, _, x_t, y_t, mask_t = future_true_data
    ...
    fig = Figure(size=(1200, 400), fontsize=16, backgroundcolor=:white, figure_padding=10)
    ...
    # Background periods
    poly!(ax, [0, max_t_o, max_t_o, 0], [y_min, y_min, y_max_val, y_max_val],
        color=(GLUCOSE_COLORS.obs_period, 0.3), label="Observation Period")
    poly!(ax, [max_t_o, x_max, x_max, max_t_o], [y_min, y_min, y_max_val, y_max_val],
        color=(GLUCOSE_COLORS.forecast_period, 0.3), label="Forecast Period")
    vlines!(ax, [max_t_o], color=("#666666", 0.8), linewidth=3, linestyle=:dash)

    # Confidence band
    band!(ax, t_p, CI_low, CI_up, color=(GLUCOSE_COLORS.confidence, 0.2))

    # Observations
    if !isempty(t_o_valid)
        scatter!(ax, t_o_valid, y_o_valid, color=GLUCOSE_COLORS.observed, markersize=12, label="Observed")
        lines!(ax, t_o_valid, y_o_valid, color=(GLUCOSE_COLORS.observed, 0.6), linewidth=2, linestyle=:dash)
    end

    # Ground truth
    if !isempty(t_p_valid)
        scatter!(ax, t_p_valid, y_t_valid, color=GLUCOSE_COLORS.truth, markersize=12, label="Ground Truth")
        lines!(ax, t_p_valid, y_t_valid, color=(GLUCOSE_COLORS.truth, 0.6), linewidth=2, linestyle=:dash)
    end

    # Predictions
    lines!(ax, t_p, μ_m[1, :, 1], color=GLUCOSE_COLORS.predicted, linewidth=3, label="Predicted (mean)")

    xlims!(ax, 0, x_max)
    ylims!(ax, y_min, y_max_val)
