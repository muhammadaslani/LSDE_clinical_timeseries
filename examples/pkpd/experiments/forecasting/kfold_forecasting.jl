...
lsde_models, lsde_params, lsde_states, lsde_performances = kfold_train_pkpd(train_loader.data, dims, k_folds, rng, config_lsde_path, "lsde", timepoints_forecast,
...
lsde_stats = assess_model_performance(lsde_performances, variables_of_interest; model_name="Latent SDE", model_type="lsde", forecast_fn=forecast_nde, plot_sample=true,
    sample_n=1, viz_fn=viz_fn_nde, models=lsde_models, params=lsde_params, states=lsde_states, data=test_loader.data,
...
lode_models, lode_params, lode_states, lode_performances = kfold_train_pkpd(train_loader.data, dims, k_folds, rng, config_lode_path, "lode", timepoints_forecast,
    loss_fn_nde, eval_fn_nde, forecast_nde, viz_fn_nde);
...
lode_stats = assess_model_performance(lode_performances, variables_of_interest; model_name="Latent ODE", model_type="lode", forecast_fn=forecast_nde, plot_sample=true,
    sample_n=1, viz_fn=viz_fn_nde, models=lode_models, params=lode_params, states=lode_states, data=test_loader.data,
...
model, θ, st = create_latent_lstm(YAML.load_file(config_latent_lstm_path)["model"], dims, rng);


latent_lstm_models, latent_lstm_params, latent_lstm_states, latent_lstm_performances = kfold_train(data, dims, k_folds, rng, config_latent_lstm_path, "latent_lstm", timepoints_forecast,
    loss_fn_lstm, eval_fn_lstm, forecast_lstm, viz_fn);

latent_lstm_stats = assess_model_performance(latent_lstm_performances, variables_of_interest; model_name="Latent LSTM", model_type="latent_lstm", forecast_fn=forecast_lstm, plot_sample=true,
    sample_n=3, viz_fn=viz_fn, models=latent_lstm_models, params=latent_lstm_params, states=latent_lstm_states, data=test_loader.data,
    timepoints=(timepoints_obs, timepoints_forecast), config=YAML.load_file(config_latent_lstm_path)["training"]["validation"]);

# Compare multiple models
model_comparison = compare_pkpd_models(Dict("LSDE" => lsde_stats, "LODE" => lode_stats, "Latent LSTM" => latent_lstm_stats), sort_by="overall");
