...
lsde_models, lsde_params, lsde_states, lsde_performances = kfold_train(data, dims, k_folds, rng, config_lsde_path, "lsde", timepoints_forecast,
                                                                                loss_fn_nde, eval_fn_nde, forecast_nde, viz_fn);

lsde_stats = assess_model_performance(lsde_performances, variables_of_interest; model_name="Latent SDE", model_type="lsde", forecast_fn=forecast_nde,
                                         plot_sample=true, sample_n=1, viz_fn=viz_fn, models=lsde_models, params=lsde_params, states=lsde_states, data=data,
...
lode_models, lode_params, lode_states, lode_performances = kfold_train(data, dims, k_folds, rng, config_lode_path, "lode", timepoints_forecast,
                                                                        loss_fn_nde, eval_fn_nde, forecast_nde, viz_fn);
...
lode_stats = assess_model_performance(lode_performances, variables_of_interest; model_name="Latent ODE", model_type="lode", forecast_fn=forecast_nde, plot_sample=true,
    sample_n=1, viz_fn=viz_fn, models=lode_models, params=lode_params, states=lode_states, data=data,
...
latent_lstm_stats = assess_model_performance(latent_lstm_performances, variables_of_interest; model_name="Latent LSTM", model_type="latent_lstm", forecast_fn=forecast_lstm, plot_sample=true,
    sample_n=1, viz_fn=viz_fn, models=latent_lstm_models, params=latent_lstm_params, states=latent_lstm_states, data=data,
