using Revise
using  Rhythm, Lux, DifferentialEquations, Random, SciMLSensitivity, ComponentArrays, Optimisers, OptimizationOptimisers, Statistics
...
function generate_dataloader(; n_samples=512, batchsize=64, split=0.6)
    ...
    U = cat(U..., dims=3)
    data = (U, X_padded, Y₁_padded, Y₂_padded, Masks₁, Masks₂)
    (train_data, val_data) = splitobs(data, at=split)
...
function viz_fn_sys_id(model, θ, st, ts, data, config; sample_n=1)
    ...
    ts *= 52.0f0
    Ey₁_masked, Ey₂_masked = Ey₁ .* mask₁, Ey₂ .* mask₂
    valid_indices = findall(mask₁[1, :, sample_n] .== 1)
    valid_ts, valid_y₁, valid_y₂ = ts[valid_indices], y₁[:, valid_indices, sample_n], y₂[1, valid_indices, sample_n]
    ...
    chemo_times = valid_ts[u[1, valid_indices, sample_n] .> 0]
    radio_times = valid_ts[u[2, valid_indices, sample_n] .> 0]
    ...
    ŷ₁_m, ŷ₁_s = selectdim(dropmean(Ey₁_masked, dims=4), 3, sample_n), selectdim(dropmean(std(Ey₁_masked, dims=4), dims=4), 3, sample_n)
    ŷ₂_m, ŷ₂_s = selectdim(dropmean(Ey₂_masked, dims=4), 3, sample_n), selectdim(dropmean(std(Ey₂_masked, dims=4), dims=4), 3, sample_n)
    Ey₂_count = rand.(Poisson.(Ey₂_masked))
    ŷ₂_m_count, ŷ₂_s_count = selectdim(dropmean(Ey₂_count, dims=4), 3, sample_n), selectdim(dropmean(std(Ey₂_count, dims=4), dims=4), 3, sample_n)

    valid_x = x[:, 1:7:end, :][:, valid_indices, sample_n]
    valid_ŷ₁_m, valid_ŷ₁_s = ŷ₁_m[:, valid_indices], ŷ₁_s[:, valid_indices]
    valid_ŷ₂_m, valid_ŷ₂_s = ŷ₂_m[1, valid_indices], ŷ₂_s[1, valid_indices]
    valid_ŷ₂_count_m, valid_ŷ₂_count_s = ŷ₂_m_count[1, valid_indices], ŷ₂_s_count[1, valid_indices]
    valid_ŷ₁_class, valid_y₁_class = onecold(valid_ŷ₁_m, Array(0:5)), onecold(valid_y₁, Array(0:5))
    ...
    lines!(ax3, valid_ts, valid_ŷ₂_m, linewidth=2, color=(atom_one_dark[:cyan], 0.5))
    band!(ax3, valid_ts, valid_ŷ₂_m .- sqrt.(valid_ŷ₂_s), valid_ŷ₂_m .+ sqrt.(valid_ŷ₂_s), color=(atom_one_dark[:cyan], 0.5), label="Inferred Tumor size")
...
train_loader, val_loader, dims, timepoints = generate_dataloader(; n_samples=256, batchsize=32, split=0.8);
...
fig=viz_fn_sys_id(model, θ_trained, st, timepoints, first(val_loader), config["training"]["validation"]; sample_n=2);
#save(joinpath(exp_path, "results_system_id_.pdf"), fig);

# validation of model prediction performance
u, x, y₁, y₂, mask₁, mask₂= first(val_loader);
x=x[:,1:7:end,:];
spl=5
...
#save(joinpath(exp_path, "results_future_prediction_.pdf"), fig)
