...
Base.@kwdef mutable struct ModelParameters
    ...
    ρ::Float64 = rand(Normal(8e-3,1e-3)) * (1 + 0.005*(age - 50)/30) * (gender == 0 ? 1.05 : 0.95) * (tumor_type == "SCLC" ? 1.1 : 1.0)
    #K::Float64 = 100.0   # Tumor carrying capacity
    K::Float64 = abs(rand(Normal(100.0,20))) * (gender == 0 ? 1.2 : 1.0) * (tumor_type == "SCLC" ? 0.8 : 1.0)
    #β_c::Float64 = 0.15  # Linear effect of chemotherapy
    β_c::Float64 = abs(rand(Normal(0.30,0.1))) * (1 - 0.003*(age - 50)) * (1 / BSA) * (tumor_type == "SCLC" ? 1.2 : 1.0)
    ω_c::Float64 = 1.0   # Chemotherapy sessions frequency (every X weeks)
    #α_r::Float64 = 0.4   # Linear effect of radiotherapy
    α_r::Float64 = rand(Normal(0.4,0.1)) * (1 - 0.002*(age - 50)) * (tumor_type == "SCLC" ? 1.2 : 1.0)
    #β_r::Float64 = 0.1   # Quadratic effect of radiotherapy
    β_r::Float64 = rand(Normal(0.1,0.05))   # Quadratic effect of radiotherapy
    ω_r::Float64 = 3.0   # Radiotherapy sessions frequency (every X weeks)
    #δ::Float64 = 0.023   # Reduced immune growth rate
    δ::Float64 = rand(Normal(0.023,0.01)) * (1 + 0.015*(age - 50)/30) * (BMI > 30 ? 0.9 : 1.0)
    #β_I::Float64 = 0.15  # Increased drug-induced immune suppression
    β_I::Float64 = rand(Normal(0.15,0.1)) * (1 - 0.002*(age - 50)) * (BMI > 30 ? 1.1 : 1.0)
#    α_I::Float64 = 0.16  # Increased radiotherapy-induced immune suppression
    α_I::Float64 = rand(Normal(0.16,0.1)) * (BMI > 30 ? 1.1 : 1.0)
