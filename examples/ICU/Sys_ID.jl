...
##loading data
data, train_loader, val_loader , time_series_dataset= load_data(;n_samples=256, batch_size=32);
inputs_data,obs_data,masks=data;
...
"output_dim" => ones(Int, size(obs_data, 1)),)
...
function eval_fn(model, θ, st, ts, data, config)
    ...
    _, Ey = predict(model, solver, y, u, ts, θ, st, config["mcmc_samples"], cpu_device(); kwargs_dict...)
    loss = sum(begin
...
function predict_future(model, θ, st, history, u, t_p, config)
    u_o, y_o, masks_o = history
    solver = eval(Meta.parse(config["solver"]))
    kwargs_dict = Dict(Symbol(k) => v for (k, v) in config["kwargs"])
    Ex, Ey_p = predict(model, solver, reverse(y_o, dims=2), u, t_p, θ, st, config["mcmc_samples"], cpu_device(); kwargs_dict...)
    return Ex, Ey_p
end
...
function viz_fn_predictions(history, predictions,ground_truth, ts_o, ts_p; sample_n=1, var_of_intrst=1)
    ...
    ts_o_val = ts_o[valid_indx_o] .* 50
    ts_p_val = ts_p[valid_indx_p] .* 50
...
viz_fn_sys_id(model, θ_trained, st, timepoints, first(val_loader), config["training"]["validation"]; sample_n=9, var_of_intrst=2);
##predicting future
spl=30;
ind_observed=1:spl; ind_predict=spl:length(timepoints);
u,y,masks=first(val_loader);
history = (u[:,ind_observed,:],  y[:, ind_observed,:], masks[:, ind_observed,:]);
...
viz_fn_predictions(history, Ey_p, ground_truth,ts_observed, ts_predict, sample_n=3, var_of_intrst=1);